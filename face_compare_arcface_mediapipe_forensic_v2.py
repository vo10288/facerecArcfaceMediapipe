...
"""
Face Compare 1:1 - Forensic Edition v2
Linux + macOS + Windows
...
- Mostra barra grafica del match con range teorico -1.00 / +1.00
- Esporta HTML
- Esporta CSV
...
APP_TITLE = "Face Compare 1:1 - MediaPipe + ArcFace Forensic v2"
...
class FaceCompareApp:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.current_similarity: Optional[float] = None
        self.tk_refs = []
    ...
    def _build_ui(self) -> None:
        ...
        tk.Label(
            top, text=APP_TITLE, bg=WINDOW_BG, fg=TEXT_FG,
        ...
        tk.Label(
            top,
            text=(
                "Originale + keypoint gialli + crop volto | "
                "MediaPipe per landmark | ArcFace per embedding e confronto"
            ),
            bg=WINDOW_BG,
            fg=MUTED,
            font=("Helvetica", 10),
        ...
        score_info = tk.Frame(self.root, bg=WINDOW_BG)
        score_info.pack(fill="x", padx=12, pady=(0, 8))

        self.score_label = tk.Label(
            score_info,
            text="Range teorico cosine similarity: min -1.00 | max +1.00 | soglia corrente: 0.55 | confronto: ArcFace embedding",
            bg=WINDOW_BG,
            fg=TEXT_FG,
            anchor="w",
            justify="left",
        ...
        self.score_label.pack(fill="x", pady=(0, 6))
    ...
    def _make_side(self, parent: tk.Widget, title: str):
        frame = tk.LabelFrame(
            parent, text=title, bg=WINDOW_BG, fg=TEXT_FG,
        ...
        info = tk.Text(
            grid, height=10, width=42, bg=PANEL_BG, fg=TEXT_FG,
            relief="flat", wrap="word", font=("Courier", 10)
        )
    ...
    def compare(self) -> None:
        try:
            ...
            self._draw_score_bar(similarity)
            ...
            lines.append("")
            lines.append("Motore di confronto biometrico: ArcFace / InsightFace")
            lines.append("Tecnica di confronto: cosine similarity tra embedding facciali")
            lines.append("MediaPipe viene usato per i keypoint gialli e i landmark, non per il match biometrico.")
            lines.append("")
            lines.append("Range teorico cosine similarity:")
            lines.append("  minimo teorico = -1.00")
            lines.append("  massimo teorico = +1.00")
            lines.append("Soglia impostabile in GUI:")
            lines.append("  minimo soglia = 0.00")
            lines.append("  massimo soglia = 1.00")
            lines.append("")
            ...
            if similarity is None:
                self.status_var.set(
                    "Confronto terminato con limitazioni: embedding non disponibile su una o entrambe le immagini."
                )
            else:
                self.status_var.set(
                    f"Confronto terminato. Similarita ArcFace: {similarity:.4f} | soglia {self._get_threshold():.2f} | {verdict}"
                )
    ...
    def _draw_score_bar(self, similarity: Optional[float]) -> None:
        ...
        if similarity is not None:
            ...
            if similarity >= threshold:
                color = "#1db954"
                verdict = "Compatibilita alta"
            elif similarity >= max(0.0, threshold - 0.20):
                color = "#f4c542"
                verdict = "Compatibilita media"
            else:
                color = "#e74c3c"
                verdict = "Compatibilita bassa"

            canvas.create_rectangle(left, top, xpos, bottom, fill=color, outline="")
            canvas.create_oval(xpos - 8, mid_y - 8, xpos + 8, mid_y + 8, fill="#ffffff", outline="#000000", width=1)
            canvas.create_text(
                left, top - 12,
        ...
        self.score_label.configure(
            text=(
                f"Range teorico cosine similarity: min -1.00 | max +1.00 | "
                f"soglia corrente: {threshold:.2f} | confronto: ArcFace embedding"
            )
        )
...
<div class='card'>
<h2>Spiegazione tecnica del punteggio</h2>
...
Nel programma:
- MediaPipe viene usato per rilevare e disegnare i keypoint gialli e i landmark
- ArcFace viene usato per il confronto biometrico vero e proprio

Interpretazione pratica:
- valori piu alti indicano maggiore similarita tra gli embedding
- la soglia corrente definisce quando il match viene considerato alto
- punteggi intermedi richiedono valutazione prudente
- il punteggio da solo non costituisce prova di identita</pre>
</div>
